function insertGuide(page_call_property) {
  ...
  const { title, guide } = getPageTitleAndGuide(
    newData["ページタイプ"],
  ...
  target.innerHTML = `
    ...
    newData["ページタイプ"] == "school"
      ? `
            ${
              page_call_property["講師名"]
                ? ` > ${page_call_property["講師名"]}先生`
                : ""
            }`
      : `
            ${
              page_call_property["教室名"]
                ? ` > ${page_call_property["教室名"]}`
                : ""
            }`
  }
        ...
        ${
          ...
          (page_call_property["callback"] == "profile_page" &&
        ...
        <div class="shortcut-buttons">
            ${
              page_call_property["callback"] == "shift_page"
                ? ""
                : `<span class="shortcut-button" id="sb-shift" data-callback="shift_page">${
                    newData["ページタイプ"] == "school"
                      ? `シフト依頼`
                      : `シフト管理`
                  }へ<i class="fa-regular fa-circle-right"></i></span>`
            }
            ${
              page_call_property["callback"] == "kintai_page"
                ? ""
                : `<span class="shortcut-button" id="sb-kintai" data-callback="kintai_page">勤怠確認へ<i class="fa-regular fa-circle-right"></i></span>`
            }
            ${
              page_call_property["callback"] == "profile_page" ||
              newData["ページタイプ"] == "teacher"
                ? ""
                : `<span class="shortcut-button" id="sb-profile" data-callback="profile_page">プロフィールへ<i class="fa-regular fa-circle-right"></i></span>`
            }
            ${
              page_call_property["callback"] == "chat_page"
                ? ""
                : `<span class="shortcut-button" id="sb-chat" data-callback="chat_page">メッセージへ<i class="fa-regular fa-circle-right"></i></span>`
            }
        </div>`
        }
  ...
  if (
    ...
    const formURL =
      "https://docs.google.com/forms/d/e/1FAIpQLSdshC5GsKHZhps40FNkEShsnnat6-B4Y_EQdRHk0XlXpwV9mg/viewform?usp=pp_url";
...
function getPageTitleAndGuide(pageType, callback) {
  let titlesAndGuides = {
    school: {
      //教室ページの定義ーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
      top_page: {
        title: "教室トップページ",
        guide:
          "スポットバイトのご利用ありがとうございます。<br>講師プロフィール・シフト確認は左のタブをクリックしてください。<br>勤務確定／勤務依頼中のシフトは下記よりご確認ください。",
      },
      shift_page: {
        title: "シフト依頼ページ",
        guide: `
      ...
      profile_page: {
        title: "講師プロフィール",
        guide:
          "登録されているプロフィール情報が表示されます。<br>指導可能科目は、講師登録時の任意選択となっています。最新の指導可能科目は講師に直接ご確認ください。",
      },
      chat_page: {
        title: "講師連絡ページ",
    ...
    teacher: {
      //講師ページの定義ーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
      top_page: {
        title: "講師トップページ",
        guide:
          "このページでは 出勤 / 退勤 の報告とこれからの勤務予定の確認ができます。教室からの勤務依頼への回答も可能です。<br>まずは、スケジュール提出を、左の<span>シフト管理ページ</span>よりお願い致します。",
      },
      shift_page: {
        title: "シフト管理ページ",
        guide: `
