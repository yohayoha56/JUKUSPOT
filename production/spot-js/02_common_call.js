...
let menuData = [
  ...
  {
    id: "shift_page",
    title: isSchool ? "シフト依頼ページ" : "シフト管理ページ",
    class: "group-menu",
  },
  { id: "kintai_page", title: "勤怠確認ページ", class: "group-menu" },
  {
    id: "profile_page",
    title: isSchool ? "講師プロフィール" : "マイプロフィール",
    class: isSchool ? "group-menu" : "single-menu",
  },
  {
    id: "chat_page",
    title: isSchool ? "講師とメッセージ" : "教室とメッセージ",
    class: "group-menu",
  },
];
...
if (isSchool) {
  ...
  childElementsData = lastNameList.map(
    (lastName, index) => `${lastName}${firstNameList[index]}`
  );
} else {
  childElementsData = newData["教室名一覧"];
}
const childElementsId = isSchool
  ? newData["講師ID一覧"]
  : newData["教室ID一覧"];
...
if (
  menuData.map((menu) => menu.id).includes(paramsP) &&
  childElementsId.includes(paramsID)
) {
...
document
  ...
  .insertAdjacentHTML("afterbegin", '<div id="overlay"></div>');
...
function call_fetchData(page_call_property) {
  ...
  switch (page_call_property["callback"]) {
    case "profile_page":
      url =
        "https://script.google.com/macros/s/AKfycbxcs2Vwu5jn_aBo5KzM6vdiO1ZNc2wb0ipcGW4-GhTr5YxAnVqmLT1596K665nqCwiNJQ/exec";
      break;
    case "chat_page":
      url =
        "https://script.google.com/macros/s/AKfycbxU7pF3PXCoyKKBZQDVFWLm4z3GH7j0Wbl4FsBFiBK8A2mmr4AVj94DWi9aZIjcnj6JEw/exec";
      break;
    case "top_page":
      url =
        "https://script.google.com/macros/s/AKfycbyHtQ0lHZ7K1uxmvTWhhDs9yohvMBJskH8klBKRQgJrm3_FBBPue1zqZid3Jo49Ua25/exec";
      break;
    default:
      url =
        "https://script.google.com/macros/s/AKfycbzBT17jaCrN-NFAuUo4ZZyoEdUWlpWhR0s6BYOHDZEhMZMcdPxKE3jOJYhMzSiuUjM0/exec";
  }
